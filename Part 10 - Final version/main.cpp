...
struct Dilemme {
  const char* question;
  const char* option_rouge;
  const char* option_bleue;
  const char* reponse_rouge;
  const char* reponse_bleue;
};

// Liste des dilemmes (vous pouvez les personnaliser)
const Dilemme dilemmes[] = {
  // Dilemme 1 - Style Matrix classique
  {
    "Tu te réveilles dans un monde étrange. Devant toi, deux pillules.",
    "Pillule rouge: Découvrir la vérité sur ce monde, aussi dure soit-elle.",
    "Pillule bleue: Retourner à une vie normale, en oubliant ce que tu as vu.",
    "Tu as choisi la vérité. Bienvenue dans le désert du réel. C'est moins confortable mais au moins tu es libre!",
    "Tu préfères l'ignorance? Très bien. Retour à ta simulation préférée. N'oublie pas de payer tes factures virtuelles!"
  },
  
  // Dilemme 2 - Dilemme moral
  {
    "Tu découvres un bouton qui peut résoudre la faim dans le monde, mais...",
    "Pillule rouge: Éliminer la faim mondiale, mais tu devras renoncer à ton plat préféré pour toujours.",
    "Pillule bleue: Garder ton plat préféré, et laisser le monde tel qu'il est.",
    "Noble sacrifice! Tu ne mangeras plus jamais de pizza/chocolat/[insérer plat], mais des millions de personnes te remercient.",
    "Mmm... Égoïste, mais au moins tu pourras encore savourer ce délicieux plat pendant que d'autres souffrent. Bon appétit!"
  },
  
  // Dilemme 3 - Dilemme absurde/humoristique
  {
    "Un alien te propose de visiter sa planète, mais il y a un choix à faire.",
    "Pillule rouge: Partir avec lui, mais tu auras des antennes sur la tête pour toujours.",
    "Pillule bleue: Rester sur Terre, mais tu seras le seul à savoir que les aliens existent.",
    "Bienvenue parmi nous! Les antennes te vont très bien, et tu captes même la radio gratuitement!",
    "Tu as choisi la solitude de la connaissance. Personne ne te croira jamais quand tu parleras des aliens..."
  }
};

int dilemme_actuel = 0;
bool jeu_en_cours = false;
bool attente_reponse = false;
unsigned long temps_dernier_dilemme = 0;
const unsigned long delai_entre_dilemmes = 5000;
...
void afficher_dilemme(int index) {
  Serial.println("\n\n============================================================");
  Serial.println("                      MATRIX DILEMMA");
  Serial.println("============================================================");
  Serial.println();
  Serial.println(dilemmes[index].question);
  Serial.println();
  Serial.println("------------------------------------------------------------");
  Serial.print("OPTION ROUGE: ");
  Serial.println(dilemmes[index].option_rouge);
  Serial.println();
  Serial.print("OPTION BLEUE: ");
  Serial.println(dilemmes[index].option_bleue);
  Serial.println("------------------------------------------------------------");
  Serial.println();
  Serial.println("Appuie sur le bouton et dis 'ROUGE' ou 'BLEU' pour choisir...");
  Serial.println("============================================================\n");
}

void afficher_reponse(int index, bool est_rouge) {
  Serial.println("\n\n------------------------------------------------------------");
  if (est_rouge) {
    Serial.println("TU AS CHOISI LA PILLULE ROUGE!");
    Serial.println(dilemmes[index].reponse_rouge);
    digitalWrite(ledRPin, HIGH);
    digitalWrite(ledBPin,LOW);
  } else {
    Serial.println("TU AS CHOISI LA PILLULE BLEUE!");
    Serial.println(dilemmes[index].reponse_bleue);
  ...
  // Passer au dilemme suivant après un délai
  temps_dernier_dilemme = millis();
...
void setup() {
  ...
  Serial.println("Initialisation du jeu Matrix Dilemma...");
  Serial.println("Prépare-toi à faire des choix qui changeront ton destin!");
  dilemme_actuel = 0;
  jeu_en_cours = true;
  attente_reponse = false;
  
  // Afficher le premier dilemme au démarrage
  afficher_dilemme(dilemme_actuel);
  attente_reponse = true;
}

void loop() {
  // Gestion du jeu des dilemmes
  if (jeu_en_cours) {
    // Si on est en attente d'une réponse
    if (attente_reponse) {
      // Vérification de l'état du bouton (pression pour déclencher la reconnaissance vocale)
      bool buttonState = digitalRead(buttonPin);
      
      // Si le bouton est pressé (front montant)
      if (buttonState == HIGH && lastButtonState == LOW && !hasPrinted) {
        Serial.println("Bouton pressé - Écoute en cours...");
        delay(250); // Anti-rebond
        
        // Capture et analyse audio
        audioToMFCC();
        NN_work(); // Cette fonction détermine si c'est "Rouge" ou "Bleu"
        
        // Récupérer le résultat du réseau neuronal
        bool choix_rouge = (output[0] > 0.05); // True si rouge, False si bleu
        
        // Afficher la réponse correspondant au choix
        afficher_reponse(dilemme_actuel, choix_rouge);
        
        // Passer au dilemme suivant
        dilemme_actuel = (dilemme_actuel + 1) % (sizeof(dilemmes) / sizeof(dilemmes[0]));
      ...
      // Réinitialise l'état hasPrinted quand le bouton est relâché
      if (buttonState == LOW) {
        hasPrinted = false;
      }
    }
    // Si on est entre deux dilemmes (délai entre les dilemmes)
    else {
      unsigned long temps_actuel = millis();
      if (temps_actuel - temps_dernier_dilemme >= delai_entre_dilemmes) {
        // Afficher le prochain dilemme
        digitalWrite(ledBPin,LOW);
        digitalWrite(ledRPin,LOW);
        afficher_dilemme(dilemme_actuel);
  ...
  // Si le jeu est terminé, on pourrait ajouter un code pour le redémarrer
  else {
    // Vérifier si l'utilisateur veut redémarrer le jeu
    bool buttonState = digitalRead(buttonPin);
    if (buttonState == HIGH && lastButtonState == LOW) {
      Serial.println("Redémarrage du jeu...");
      dilemme_actuel = 0;
      jeu_en_cours = true;
      digitalWrite(ledBPin, LOW);
      digitalWrite(ledRPin, LOW);
